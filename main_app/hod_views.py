...
def add_staff(request):
    ...
    context = {'form': form, 'page_title': 'Nhân viên mới'}
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                ...
                user.save()
                messages.success(request, "Đã thêm nhân viên " + user.first_name + " " + user.last_name)
                return redirect(reverse('add_staff'))

            except Exception as e:
                messages.error(request, "Không thể thêm nhân viên mới! " + str(e))
        else:
...
def add_student(request):
    ...
    context = {'form': student_form, 'page_title': 'Thêm học sinh'}
    if request.method == 'POST':
        if student_form.is_valid():
            ...
            try:
                ...
                user.save()
                messages.success(request, "Đã thêm học sinh " + user.first_name + " " + user.last_name)
                return redirect(reverse('add_student'))
            except Exception as e:
                messages.error(request, "Không thể thêm học sinh mới! " + str(e))
        else:
            messages.error(request, "Không thể thêm nhân viên mới! ")
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request, "Thêm mới không thành công...")
        else:
            messages.error(request, "Thêm mới không thành công...")
...
def add_subject_fee(request):
    form = SubjectFeeForm(request.POST or None)
    context = {
        'form': form,
        'page_title': 'Thêm học phí'
    }
    if request.method == 'POST':
        if form.is_valid():
            course = form.cleaned_data.get('course')
...
def edit_staff(request, staff_id):
    ...
    context = {
        'form': form,
        'staff_id': staff_id,
        'page_title': 'Chỉnh sửa thông tin nhân viên'
    }
    if request.method == 'POST':
        if form.is_valid():
            first_name = form.cleaned_data.get('first_name')
            ...
            try:
                ...
                messages.error(request, "Cập nhật không thành công... " + str(e))
        else:
            messages.error(request, "Vui lòng điền đầy đủ thông tin yêu cầu...")
    else:
...
def edit_student(request, student_id):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            messages.error(request, "Please Vui lòng điền đầy đủ thông tin yêu cầu...!")
    else:
...
def add_session(request):
    ...
    context = {'form': form, 'page_title': 'Thêm nhóm'}
    if request.method == 'POST':
        if form.is_valid():
            try:
                form.save()
...
def manage_session(request):
    ...
    context = {'sessions': sessions, 'page_title': 'Quản lý nhóm'}
...
def edit_session(request, session_id):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            messages.error(request, "Biểu mẫu đã gửi không hợp lệ... ")
            return render(request, "hod_template/edit_session_template.html", context)
...
def staff_feedback_message(request):
    if request.method != 'POST':
        ...
        context = {
            'feedbacks': feedbacks,
            'page_title': 'Góp ý của nhân viên'
        }
...
def admin_notify_staff(request):
    ...
    context = {
        'page_title': "Gửi thông báo cho nhân viên",
        'allStaff': staff
    }
...
def admin_notify_student(request):
    ...
    context = {
        'page_title': "Gửi thông báo cho học sinh",
        'students': student
    }
